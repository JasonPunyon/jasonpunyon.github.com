...
task :preview do
  ...
  buildrPid = Process.spawn("Rscript build.R")
  jekyllPid = Process.spawn({"OCTOPRESS_ENV"=>"preview"}, "jekyll build --watch")
  compassPid = Process.spawn("compass watch")
  rackupPid = Process.spawn("rackup --port #{server_port}")

  trap("INT") {
    [buildrPid, jekyllPid, compassPid, rackupPid].each { |pid| Process.kill(9, pid) rescue Errno::ESRCH }
    exit 0
  }

  [buildrPid, jekyllPid, compassPid, rackupPid].each { |pid| Process.wait(pid) }
end
...
desc "Begin a new Rmd in #{source_dir}_R"
task :new_rmd, :title do |t, args|
  ...
  raise "### You haven't set anything up yet. First run `rake install` to set up an Octopress theme." unless File.directory?(source_dir)
  mkdir_p "#{source_dir}/#{posts_dir}"
  filename = "#{source_dir}/_R/#{Time.now.strftime('%Y-%m-%d')}-#{title.to_url}.Rmd"
  if File.exist?(filename)
    abort("rake aborted!") if ask("#{filename} already exists. Do you want to overwrite?", ['y', 'n']) == 'n'
  end
  puts "Creating new post: #{filename}"
